...
class LoadImagesAndLabels(Dataset):
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        pbar.close()
        # pbar = tqdm(zip(self.im_files, self.label_files, repeat(prefix)), desc='Scanning images', total=len(self.img_files))
    ...
    def __getitem__(self, index):
        ...
        if nl:
            ...
            # showlabels(img, labels[:, 1:5], labels[:, 5:17])
        # Convert
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            nlb = lb[:, :5]
            if nl:
                assert lb.shape[1] == 17, f'labels require 5 columns, {lb.shape[1]} columns detected'
                ...
                ne = 1  # label empty
                lb = np.zeros((0, 17), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, 17), dtype=np.float32)
